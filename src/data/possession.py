...
def parse_possession(in_data, out_data):
    ...
    plays = out_data["plays"][
        int(possession.play_start_idx) : int(possession.play_end_idx)
    ]
    ...
    possession.offense_fouls_left = (
        prev.defense_fouls_left if switched else prev.offense_fouls_left
    )
    possession.defense_fouls_left = (
        prev.offense_fouls_left if switched else prev.defense_fouls_left
    )
    ...
    possession.foul_change = sum(
        [
            int(play.counts_towards_penalty)
            for play in raw_possession.events
    ...
    possession.start_time = time_to_seconds(
        raw_possession.start_time, possession.period
    )
