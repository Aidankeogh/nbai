...
def load_raw_data(
    ...
    for in_season in season_iter:
        ...
        game_iter = StatsNbaGameFinderLoader(
            "nba", year_string, in_season[1], "file", "cache/data"
        ).items
        in_data["season"] = in_season
        for in_game in game_iter:
            possession_iter = StatsNbaPossessionLoader(
                in_game.data["game_id"], "file", "cache/data"
            ).items
